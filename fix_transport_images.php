...
$transports = \App\Models\Transportation::all();
$save_path = 'e:\\اخر تحديث\\kemet-frontend-main\\public\\images\\downloads\\transportations\\';
...
$count = 0;
foreach ($transports as $idx => $item) {
    if ($idx >= count($external_images)) break;
    $url = $external_images[$idx];
    ...
    try {
        ...
        if ($response->successful()) {
            ...
            $newFilename = 'transport_' . $item->id . '_' . Str::random(5) . '.' . $ext;
            
            // Save to frontend public path
            file_put_contents($save_path . $newFilename, $response->body());
            
            // Update the DB
            $item->image = '/images/downloads/transportations/' . $newFilename;
            $item->save();
            $count++;
            echo "Downloaded and mapped " . $item->type . " image to " . $item->image . "\n";
        } else {
             echo "Failed to download $url - Status: " . $response->status() . "\n";
        }
    } catch (\Exception $e) {
         echo "Exception on $url: " . $e->getMessage() . "\n";
    }
}

echo "Successfully mapped and localized $count transportation images.";
